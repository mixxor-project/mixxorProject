...
[System.Serializable]
public class CharaFacePosStore //캐릭터의 페이스 이미지 posisions를 관리하는 함수
{
...
public class StoreText // 받아온 텍스트 리스트를 저장하는 class
{
    public string text;
    ...
    #region 클래스 추가 부분
    public StoreText(string _text, string _charaName = "", string _charaFace = "", int _charaPos = 999, string _itemContent = "", string _itemAnswer = "")
    {
        text = _text;
        ...
        Debug.Log(_text + _charaName + _charaPos + _itemContent + _itemAnswer);
    }

    public StoreText(string _text, string _itemContent, string _itemAnswer)
    {
        text = _text;
        charaName = "";
        charaFaceName = "";
...
public class StroeTextPrint : MonoBehaviour
{
    public static StroeTextPrint instance;

    //캐릭터 이미지 설정
    [Header("캐릭터 이미지 리스트")]
    public CharaFacePosStore charaImgPos;
    ...
    private void Start() //테스트를 합니다.
    {
        ...
        //texts.Add(new StoreText(true, "반갑<굵게>습니다.</굵게>", "구글에서 긁어온 배경"));
        //texts.Add(new StoreText("스푼이 다가옵니다...", "스푼", "", 0));
        //texts.Add(new StoreText("안녕하세요...", "스푼"));
        //texts.Add(new StoreText("내 이름은 스푼입니다...", "스푼", "웃음"));
        //texts.Add(new StoreText("스푼은 늘 행복합니다...", "스푼", "웃음", -1));
        //texts.Add(new StoreText("내 이름은 <강조>포크</강조>입니다...", "포크", "웃음"));
        //texts.Add(new StoreText("포크는 늘 행복합니다...", "포크", "웃음", -1));
        //texts.Add(new StoreText("나이프가 포크 앞에 끼어듭니다...", "나이프", "기본", -1));
        //texts.Add(new StoreText("하지만 당신이 무시하면... 스푼은 화냅니다...", "스푼", "분노", 1));
        //texts.Add(new StoreText("스푼이 화를 냅니다...", "스푼", "분노"));
        //texts.Add(new StoreText("#모두 퇴장"));
        //texts.Add(new StoreText(true, "스푼은 갔습니다...", "구글에서 긁어온 바닷가"));
        //texts.Add(new StoreText("내 이름은 포크입니다...", "포크", "웃음"));
        //texts.Add(new StoreText("나이프가 포크 앞에 끼어듭니다...", "나이프", "기본", -1));
        //texts.Add(new StoreText("#나이프/포크 퇴장"));
        //texts.Add(new StoreText("나이프가 저리 갑니다...", "나이프", "기본", 1));
        //texts.Add(new StoreText("#나이프 퇴장"));
    ...
    public void TextStart(StoreText[] _texts) //StoreText[]을 받고 텍스트 이벤트 자체를 실행시켜주는 함수
    {
        nowTextActive = true;
        for (int i = 0; i < _texts.Length; i++)
        {
            storeTextList.Add(_texts[i]);
        }
    ...
    IEnumerator TextListCoroutine() //받은 텍스트 리스트를 가지고 텍스트 자체를 출력해주는 함수
    {
        ...
        while (true)
        {
            if (storeTextList.Count <= 0) break;
